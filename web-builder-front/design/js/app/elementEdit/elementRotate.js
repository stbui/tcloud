// liumingren 2015.11.9

define(['smartMenu','createColorStyle'],function(smartMenu,_ccs) {

	var rotate = {
		// 旋转
		init:function () {
			var self = this;
			this.bindRotation("[data-elemandgroup='true']",".rotationdiv");
			$(document).on("rotate.show",function (e,data) {
				data.element.find(".rotationdiv").remove().end().append("<div class='rotationdiv'><span class='rotationClear'></span><span class='rotationAngle'></span><div class='rotationdivend'></div></div>");
			...
			$(document).on("clearRotate",".wqdelementEdit",function (e) {
				e.stopPropagation();
				var $elm = $(this);
		...
		*/
		bindRotation:function (elm,handle) {
			...
			$(document).on("mousedown",handle,function (e) {
				if($(e.target).hasClass("rotationClear")) return false;
				var that = this,
