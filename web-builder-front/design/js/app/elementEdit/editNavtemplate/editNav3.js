...
define(['utility','createColorStyle','popupEdit','popupBase'],function(utility,createColorStyle,popupEdit,popupBase) {
    ...
    wqdNavTemp.init = function () {
        ...
        $(document).on("elmenentEdit",function (e,data) {
            if(data && data.element && data.element.attr("data-elementtype") == "navTemplate3"){
                self.editDom = data.element|| $(".wqdelementEdit.onEditing");
                setTimeout(function(){
                    //加载json
                    $.getJSON('../js/app/JSON/navtemplate/designControlNav3.json', function(json, textStatus) {
    ...
    wqdNavTemp.bindEvent=function () {
        ...
        editor.on("changeColor", ".wqdColorPicker",function(){
            var that = $(this),
                ...
                elemId = dom.attr("id") || "",
                colorVal = that.val(),
                cssstyle = {"title-color":{"color":colorVal},"title-hovercolor":{"color":colorVal+"!important"},"title-selectcolor":{"color":colorVal+"!important"},"title-bg":{"background-color":colorVal},"title-hoverbg":{"background-color":colorVal+"!important"},"title-selectbg":{"background-color":colorVal+"!important"}},
                selecter = {"title-color":".navTop_list li a","title-hovercolor":".navTop_list li:hover a","title-selectcolor":".navTop_list li.active a","title-bg":".navTop_list li","title-hoverbg":".navTop_list li:hover","title-selectbg":".navTop_list li.active"};
        ...
        editor.find(".slider").each(function(i){
            var That = $(this),
                type = That.attr("data-type"),
                typeObj1 = {"font-size":20,"menu-redius":8,"menu-width":100},
                typeObj2 = {"font-size":12,"menu-redius":0,"menu-width":10};
            utility.range({
                slider : That,
    ...
    wqdNavTemp.changeSize = function(type,val){
        var dom = wqdNavTemp.editDom,
            ...
            thisVal = val + "px",
            elemId = dom.attr("id") || "",
            selector = type == "menu-redius" ? ".navTop_list li:before, #"+elemId+" .navTop_list li:after" : ".navTop_list li a",
            cssObj = {"font-size":{"font-size":thisVal},"menu-redius":{"border-top-left-radius":thisVal,"border-top-right-radius":thisVal},"menu-width":{"padding-left":thisVal,"padding-right":thisVal}};
        dataStyle += type + ":" + val + ";";
        dom.attr("data-style",dataStyle);
        createColorStyle.styleInit(elemId,selector,cssObj[type]);
        $(document).trigger("appSetCatch");
    }
    //去掉已存在data-style项
