define(['utility'],function(utility) {
	...
	wqdNavigate.insideEvent = function(){
		...
		//监听导航添加事件
		$(document).on("wqdAddNavigate",function(e,isTemp){
			if(isTemp) return;
	...
	wqdNavigate.outsideEvent = function(){
		...
		var isPhoneNav = $(".wqdPhoneNav").length;
		var phoneNavEvent = isPhoneNav ? "click" : "mouseenter";
		var t1 = null;
		$(document).on(phoneNavEvent,".wqdNavWrap .wqdNavLi",function(){
			if (t1 == null){
					t1 = new Date().getTime();
				}else{
	...
	wqdNavigate.createAddBtn = function(){
		...
		var addbtn = $('<a class="wqdAddsecondnav" href="javascript:void(0);">+</a>'),
			elmToP = isSideNav ? that.offset().top : (that.outerHeight()+that.offset().top > that.parents(".wqdTopNavbox").offset().top+that.parents(".wqdTopNavbox").height() ? that.parents(".wqdTopNavbox").offset().top+that.parents(".wqdTopNavbox").height()-1 : that.offset().top + that.outerHeight()-1),
	...
	wqdNavigate.deleteNav = function(){
		var that = $(this);
		$(".wqdNavMenuWrap, .wqdAddsecondnav").remove();
		that.find(".wqdNavList li").each(function(){
			var thisMark = $(this).attr("wqdNavmark") || "wqdNomark",
			secondObj = $(".wqdSecondNavbox"+"[wqdnavmark="+thisMark+"]");
	...
	wqdNavigate.deleteNewNavmenu = function(event){
		...
		var $secondNavbox=$(".wqdSecondNavbox");
		var $navMark=$(".wqdSecondNavbox" + "[wqdnavmark=" + thisMark + "]");
		if ($secondNavbox.length) {
			if (obj.index() == 0){
				$secondNavbox.each(function () {
					var x=40,y=58;
					navBox.call(this,x,y)
				})
			}else{
				$secondNavbox.each(function () {
					var x=40,y=86;
		...
		function navBox(x,y){
			(parseFloat($(this).css("left")) > parseFloat($navMark.css("left")))&&$(this).css("left", parseFloat($(this).css("left")) - y + "px");
			(parseFloat($(this).css("top")) > parseFloat($navMark.css("top")))&&$(this).css("top", parseFloat($(this).css("top")) - x + "px");
		};
		obj.remove();
		$navMark.remove();
		$(".wqdNavMenuWrap").remove();
	...
	wqdNavigate.navAutoInside = function(){
		var pageid = $(".pagedeatllist>li.on").attr("pageid"),
